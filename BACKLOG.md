...
- After `herdr-win` publishes a managed build with PATH-based pre-provisioned Windows attach, run the fresh native dependency gate from an actual WinGet-managed host install: require pre-cleanup remote capability, a digest-identical host-to-guest runtime copy with no Herdr network/cache request, Application lookup of the copied `herdr.exe` through the real SSH user's guest `PATH`, final host identity revalidation, matching version/protocol, SSH remote attach, detach, and persistent server reuse. Also prove that a stock Windows build reporting `unsupported` fails before cleanup and prints the exact `hdosys.herdr-win` WinGet action.
