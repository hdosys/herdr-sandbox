...
func TestBuildEffectivePlanInspectsGlobalStacksWithoutAWorkspace(t *testing.T) {
	...
	if strings.Join(plan.GlobalStacks, "|") != "dotnet" || len(plan.StackPackages) != 1 || len(plan.ToolVersions) != 1 ||
		plan.ToolVersions[0].Tool != "Microsoft.DotNet.SDK" || plan.ToolVersions[0].Selection != "latest stable during provisioning" ||
		plan.ToolVersions[0].Source != "stack default/latest stable" ||
		plan.StackPackages[0].PackageOwner != "current stable Microsoft.DotNet.SDK family" || !strings.Contains(plan.NextAction, "init") {
