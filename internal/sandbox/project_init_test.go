...
func TestInitializeProjectWritesDeterministicDirectStackCalls(t *testing.T) {
	project := t.TempDir()
	result, err := InitializeProject(project, []string{"zig", "dotnet", "rust", "go", "node", "nsis", "nushell", "playwright-cli", "python", "tradingview", "cpp", "java", "android"})
	if err != nil {
		t.Fatal(err)
	}
	wantLabels := "android|cpp|dotnet|go|java|node|nsis|nushell|playwright-cli|python|rust|tradingview|zig"
...
func TestProjectPlanRecognizesModernDotNetDirectCall(t *testing.T) {
	...
	data := []byte(`{"schemaVersion":3,"userStacks":[],"userTools":[],"projects":[{"name":"project","stacks":["dotnet"],"tools":[{"tool":"Microsoft.DotNet.SDK.10","version":"","series":"","source":"dotnet","projectDirectory":""}]}]}`)
	inspection, err := decodeProjectProvisioningPlan(data, workspaces)
	result := inspection.Workspaces
