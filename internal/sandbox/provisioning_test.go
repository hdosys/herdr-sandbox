...
func TestInstallPythonStackReusesRunningAppLocalCommandsInWindowsPowerShell51(t *testing.T) {
	requireExternalBoundaryTest(t, "Windows PowerShell Python command reuse")
	if runtime.GOOS != "windows" {
		t.Skip("Windows PowerShell 5.1 Python command reuse contract")
	}
	root := t.TempDir()
	toolsRoot := filepath.Join(root, "tools")
	aliasRoot := filepath.Join(toolsRoot, "python", "bin")
	if err := os.MkdirAll(aliasRoot, 0o700); err != nil {
		t.Fatal(err)
	}
	publisherPython := filepath.Join(root, "publisher-python.exe")
	pythonAlias := filepath.Join(aliasRoot, "python.exe")
	python3Alias := filepath.Join(aliasRoot, "python3.exe")
	for path, contents := range map[string]string{
		publisherPython: "publisher",
		pythonAlias:     "retained-python",
		python3Alias:    "retained-python3",
	} {
...
$errors = $null
$ast = [Management.Automation.Language.Parser]::ParseFile('%s', [ref]$tokens, [ref]$errors)
$definition = $ast.Find({ param($node) $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -ceq 'Install-PythonStack' }, $true)
if ($null -eq $definition) { throw 'Missing Install-PythonStack.' }
$functionText = $definition.Extent.Text.Replace('C:\HerdrSandbox\tools', '%s')
Invoke-Expression $functionText
function Get-ProvisioningToolSeries { param($Tool, $Requested) if ([string]::IsNullOrWhiteSpace($Requested)) { '3.14' } else { $Requested } }
function Get-ProvisioningToolVersion { param($Tool, $Requested) if ([string]::IsNullOrWhiteSpace($Requested)) { '3.14.7' } else { $Requested } }
function Resolve-StackPythonPackage { param($Series, $Version) [pscustomobject]@{ Series = $Series; Version = $Version } }
function Get-ProvisioningWinGetMetadata { [pscustomobject]@{ Version = '3.14.7' } }
function Install-ProvisioningCachedPackage {}
function Invoke-ProvisioningNative { @('Python 3.14.7') }
function Assert-ProvisioningAuthenticodeSignature {}
function Get-AuthenticodeSignature { [pscustomobject]@{ SignerCertificate = [pscustomobject]@{ Subject = 'CN=Python Software Foundation' } } }
function Add-ProvisioningMachinePath {}
$script:pythonLookupCount = 0
function Wait-ProvisioningCommandAvailable {
    param($Role, $Name, $CommandSourceExclusion)
    if ($Name -ceq 'python3.exe') { return $python3Alias }
    $script:pythonLookupCount++
    if ($script:pythonLookupCount -eq 1) { return $publisherPython }
    return $pythonAlias
}
$script:checkedPythonCommands = @()
function Assert-ProvisioningCommand {
    param($Role, $Name, $VersionArguments, $ExpectedPattern)
    $script:checkedPythonCommands += $Name
    return 'Python 3.14.6'
}
$pythonLock = [IO.File]::Open($pythonAlias, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::Read)
$python3Lock = [IO.File]::Open($python3Alias, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::Read)
try {
    Install-PythonStack
    if ([IO.File]::ReadAllText($pythonAlias) -cne 'retained-python' -or
        [IO.File]::ReadAllText($python3Alias) -cne 'retained-python3') {
        throw 'Install-PythonStack replaced an existing app-local command.'
    }
    if (($script:checkedPythonCommands -join '|') -cne 'python.exe|python3.exe') {
        throw "Install-PythonStack did not execute both retained commands: $($script:checkedPythonCommands -join '|')"
    }
} finally {
    $python3Lock.Dispose()
    $pythonLock.Dispose()
}
`, quote(publisherPython), quote(pythonAlias), quote(python3Alias), quote(stackPath), quote(toolsRoot))
	powerShell := mustWindowsPowerShellPath(t)
	command := hiddenCommand(powerShell, "-NoLogo", "-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden", "-EncodedCommand", encodePowerShell(script))
	if output, err := command.CombinedOutput(); err != nil {
		t.Fatalf("Python command reuse contract: %v: %s", err, output)
	}
}
...
func TestEnsureGlobalProvisioningSeedsUserWithoutOverwriting(t *testing.T) {
	...
	if !bytes.Contains(seededContents, []byte(`"modelsDirectory": ""`)) {
		t.Fatalf("seeded config does not expose the optional shared models directory: %s", seededContents)
	}
