...
func OpenConfiguration() (string, error) {
...
func openConfigurationAt(configurationRoot string, openFile func(string) error) (string, error) {
	...
	if err := openFile(path); err != nil {
		return path, fmt.Errorf("open configuration %s with its registered application: %w", path, err)
	}
	return path, nil
}
