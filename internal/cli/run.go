...
const usage = `Usage:
  ...
  sandbox --version
  sandbox plan
  sandbox init [--stack android|cpp|dotnet|go|java|node|nsis|nushell|playwright-cli|python|rust|tradingview|zig|all|handy|herdr|hyperframes|python-ai]...
...
const stackSelectionHelp = "android, cpp, dotnet, go, java, node, nsis, nushell, playwright-cli, python, rust, tradingview, zig, all, handy, herdr, hyperframes, python-ai"
...
func runWithCommandDependencies(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer, dependencies commandDependencies) int {
	...
	case "--installer-clean-uninstall":
		...
		if len(args) == 3 && args[1] == "--installer-lifecycle-lock-held" && args[2] == "--delete-configuration" {
			...
			lockHeld = true
		} else if len(args) == 2 && args[1] == "--installer-lifecycle-lock-held" {
			lockHeld = true
		} else {
			fmt.Fprintln(stderr, "sandbox: installer clean uninstall requires --installer-lifecycle-lock-held and accepts only --delete-configuration after it")
			return 2
		}
