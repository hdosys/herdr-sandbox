...
Function un.DeleteConfigurationPage
    ...
    ${If} $DeleteConfigurationUnavailable == "1"
        ...
        Pop $0
    ${Else}
        ${NSD_CreateLabel} 0 0 100% 72u "Uninstall removes ${APP_DISPLAY_NAME} machine-local state, SSH integration, the configured package/tool cache, and its app-owned configuration references. A running Sandbox stays open but becomes unmanaged; close it manually when finished. Select this option to also remove ${APP_CONFIG_FILE} and ${APP_USER_SCRIPT}. Project ${APP_PROJECT_DIRECTORY} profiles are not removed."
        Pop $0
...
Section "Install"
    ...
    DetailPrint "Creating default configuration and refreshing its references..."
