...
func TestPrepareNativeAllStacksFixtureIsCredentialFreeAndComplete(t *testing.T) {
	...
	for _, call := range []string{"Install-DotNetStack", "Install-GoStack", "Install-HerdrStack", "Install-NodeStack", "Install-PlaywrightCLIStack"} {
	...
	for _, required := range []string{"playwright-chromium", "playwright-cli-version", "mmlmfjhmonkocbjadbfplnigmagldckm", `C:\HerdrSandbox\tools\playwright`, "PLAYWRIGHT_BROWSERS_PATH", "bun-run", "python3-version", "herdr-just-toolchain", "python3-just-ok", "bun-just-ok", "cargo-nextest-version", "just-version", "sh-run", "LIBGHOSTTY_VT_ZIG_OUT_DIR"} {
